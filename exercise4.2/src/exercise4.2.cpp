...
static void toggleTC74Standby(Sleeper &, int);
static int checkStandby(Sleeper &);
...
static int checkStandby(Sleeper &s){
	s.sleep(2);
	uint8_t ret = 0;
...
static void toggleTC74Standby(Sleeper &s, int toStandby){
	s.sleep(2);
	ITM_wrapper iw;
	...
	SetupXferRecAndExecute(I2C_TEMP_ADDR_7BIT, sendbuf, 2,
			&sendbuf[1], 0);
}
...
int main(void)
{
	...
	while (1) {
		if (timer >= 5000 && !standby){
			standby = true;
			toggleTC74Standby(s, standby);
		}
